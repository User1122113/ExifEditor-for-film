...
CAMERA_PROFILE_DIR = os.path.join(APP_DIR, "Camera Profile")


def _get_icon_path() -> str:
    candidates = [
        os.path.join(APP_DIR, "Film_Writer_icon.ico"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "Film_Writer_icon.ico"),
    ]
...
def _ensure_resource_dir(target_dir: str, seed_dir: str | None) -> tuple[bool, bool]:
    created = False
    ...
    try:
        if seed_dir and os.path.isdir(seed_dir):
            if not os.listdir(target_dir) and os.listdir(seed_dir):
...
def _init_resource_dirs() -> list[str]:
    seed_base = os.path.dirname(os.path.abspath(__file__))
    seed_fonts = os.path.join(seed_base, "fonts")
    seed_profiles = os.path.join(seed_base, "Camera Profile")
    fonts_created, fonts_seeded = _ensure_resource_dir(FONTS_DIR, seed_fonts)
    profiles_created, profiles_seeded = _ensure_resource_dir(CAMERA_PROFILE_DIR, seed_profiles)
    messages: list[str] = []
    if fonts_created:
        messages.append(f"fonts 폴더 생성: {FONTS_DIR}")
    if profiles_created:
        messages.append(f"Camera Profile 폴더 생성: {CAMERA_PROFILE_DIR}")
    if fonts_seeded:
        messages.append("fonts 기본 리소스를 외부 폴더로 복사했습니다.")
    if profiles_seeded:
        messages.append("Camera Profile 기본 리소스를 외부 폴더로 복사했습니다.")
    return messages


# 기본 폰트 경로: fonts 폴더에 E1234.ttf를 배치하세요.
DEFAULT_FONT_PATH = os.path.join(FONTS_DIR, "E1234.ttf")
...
class App(tk.Tk):
    def __init__(self):
        ...
        created_messages = _init_resource_dirs()
        if created_messages:
            try:
    ...
    def _build_ui(self):
        ...
        required = btns.winfo_reqwidth() + 12
        left.configure(width=required)
        paned.sashpos(0, required)
    ...
    def _export_camera_profile(self):
        ...
        profile_dir = Path(CAMERA_PROFILE_DIR)
